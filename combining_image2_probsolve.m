...
img_folder = strcat('C:\Users\mhossa89\Desktop\image processing folder\split averaged image\2 inch\A_');
...
for u_j =2
    for u_cf = 10
        for cam = 1
            ...
            img_loc = strcat('A_',num2str(u_cf),'_',num2str(u_j));
          ...
          %  if cam == 3     %third camera mapping
                ...
                for k = 1:1920
                    for  kk = 1:1155
                        averaged_image(diff3+kk,3000+k) = img3(45+kk,k);
                    end
                end
          %  end
        end
   
        jpgFileName = strcat('A_',num2str(u_cf),'_',num2str(u_j),'.tif');
